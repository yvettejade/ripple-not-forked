...
3. Validator Verification and Execution: Validators receive the transaction and perform a series of checks and state changes as a single:
   ...
   - Ledger Changes: If the proof is valid, the validators execute all of the following changes at once:
     ...
     3. The global OA (`OutstandingAmount`) is decreased by the RevealedAmount, burning the clawed-back tokens. The funds are permanently removed from circulation and are **not** credited to any account.
...
This single transaction securely and verifiably burns the holder's entire confidential balance, decreasing both OA and COA. The clawed-back tokens are permanently removed from circulation and are **not** credited to any account, including the issuer's reserve.
