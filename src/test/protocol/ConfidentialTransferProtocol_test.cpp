...
class ConfidentialTransferProtocol_test : public beast::unit_test::Suite
{
    ...
    testSerializedFields()
    {
        ...
        BEAST_EXPECT(sfIssuerEncryptionKey.fieldValue == 35);
        BEAST_EXPECT(sfAuditorEncryptionKey.fieldType == STI_VL);
        BEAST_EXPECT(sfAuditorEncryptionKey.fieldValue == 44);
        BEAST_EXPECT(sfHolderEncryptionKey.fieldType == STI_VL);
        BEAST_EXPECT(sfHolderEncryptionKey.fieldValue == 36);
        BEAST_EXPECT(sfConfidentialBalanceSpending.fieldType == STI_VL);
        BEAST_EXPECT(sfConfidentialBalanceSpending.fieldValue == 33);
        BEAST_EXPECT(sfConfidentialBalanceInbox.fieldType == STI_VL);
        BEAST_EXPECT(sfConfidentialBalanceInbox.fieldValue == 32);
        BEAST_EXPECT(sfIssuerEncryptedBalance.fieldType == STI_VL);
        BEAST_EXPECT(sfIssuerEncryptedBalance.fieldValue == 34);
        BEAST_EXPECT(sfAuditorEncryptedBalance.fieldType == STI_VL);
        BEAST_EXPECT(sfAuditorEncryptedBalance.fieldValue == 42);
        BEAST_EXPECT(sfHolderEncryptedAmount.fieldType == STI_VL);
        BEAST_EXPECT(sfHolderEncryptedAmount.fieldValue == 38);
        BEAST_EXPECT(sfIssuerEncryptedAmount.fieldType == STI_VL);
        BEAST_EXPECT(sfIssuerEncryptedAmount.fieldValue == 39);
        BEAST_EXPECT(sfAuditorEncryptedAmount.fieldType == STI_VL);
        BEAST_EXPECT(sfAuditorEncryptedAmount.fieldValue == 43);
        BEAST_EXPECT(sfZKProof.fieldType == STI_VL);
        BEAST_EXPECT(sfZKProof.fieldValue == 37);
        BEAST_EXPECT(sfSenderEncryptedAmount.fieldType == STI_VL);
        BEAST_EXPECT(sfSenderEncryptedAmount.fieldValue == 40);
        BEAST_EXPECT(sfDestinationEncryptedAmount.fieldType == STI_VL);
        BEAST_EXPECT(sfDestinationEncryptedAmount.fieldValue == 41);
        BEAST_EXPECT(sfBalanceCommitment.fieldType == STI_VL);
        BEAST_EXPECT(sfBalanceCommitment.fieldValue == 46);
        BEAST_EXPECT(sfAmountCommitment.fieldType == STI_VL);
        BEAST_EXPECT(sfAmountCommitment.fieldValue == 45);
